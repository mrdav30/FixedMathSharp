...
**Tech Stack:** .NET 8, xUnit, `Fixed64`, `Vector2d`, `Vector3d`, `Vector4d`, `FixedQuaternion`, `Fixed3x3`, `Fixed4x4`, FixedMathSharp-Unity adapter extensions.
...
- Modify: `tests/FixedMathSharp.Tests/Vector4d.Tests.cs`
- Modify: `tests/FixedMathSharp.Tests/FixedQuaternion.Tests.cs`
...
## Phase 2: Fix Real Convention Bugs Before Adding New APIs
...
- [ ] Fix `FixedQuaternion.FromDirection(Vector3d.Backward)` so the anti-parallel case returns a deterministic 180-degree rotation instead of identity. Prefer `Vector3d.Up` as the fallback axis unless it conflicts with the input convention under test.
- [ ] Add tests for `FromDirection(Vector3d.Backward)`, near-forward, and near-backward inputs.
- [ ] Audit `Vector3d.Direction` naming and behavior. The current docs say `X` is yaw and `Y` is pitch, while the formula behaves like `X` controls signed vertical pitch and `Y` controls horizontal yaw around `+Y`.
- [ ] Decide whether to rename docs only or add clearer helpers such as `DirectionFromPitchYaw` / `FromPitchYaw`. Keep existing public API stable unless a breaking-change phase is explicitly approved.
- [ ] Add tests for zero angles, positive yaw, negative yaw, positive pitch, and negative pitch using canonical `+Z` forward expectations.

Verification:

```bash
dotnet test tests/FixedMathSharp.Tests/FixedMathSharp.Tests.csproj --configuration Debug --filter "FullyQualifiedName~FixedQuaternionTests|FullyQualifiedName~Vector3dTests"
```

## Phase 3: Add Engine-Agnostic Convention Types
...
- Create: `tests/FixedMathSharp.Tests/CoordinateConventions/CoordinateConvention3d.Tests.cs`
...
## Phase 4: Clarify Core Documentation And XML Comments
...
**Files:**
- Modify: `/mnt/f/gamedevrepos/FixedMathSharp-Unity/Assets/Packages/com.mrdav30.fixedmathsharp/Runtime/Extensions/Vector3d.Extensions.cs`
- Modify: `/mnt/f/gamedevrepos/FixedMathSharp-Unity/Assets/Packages/com.mrdav30.fixedmathsharp/Runtime/Extensions/Vector2d.Extensions.cs`
- Modify: `/mnt/f/gamedevrepos/FixedMathSharp-Unity/Assets/Packages/com.mrdav30.fixedmathsharp/Runtime/Extensions/FixedQuaternion.Extensions.cs`
- Modify: `/mnt/f/gamedevrepos/FixedMathSharp-Unity/Assets/Packages/com.mrdav30.fixedmathsharp/Runtime/Extensions/Fixed4x4.Extensions.cs`
- Mirror equivalent changes under `/mnt/f/gamedevrepos/FixedMathSharp-Unity/Assets/Packages/com.mrdav30.fixedmathsharp.lean/Runtime/Extensions/`
...
## Phase 6: Add XNA/MonoGame Adapter Guidance Or Package
...
Start with Phases 1 and 2 only. They are the highest-signal work because they lock down the existing convention and fix behavior that appears incorrect independent of Unity or XNA. After that, Phase 3 can introduce the convention abstraction with much lower risk, because tests will already prove what must remain stable.
