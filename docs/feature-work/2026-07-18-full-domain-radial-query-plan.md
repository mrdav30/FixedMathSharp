...
- Modify: `../Gravitas/src/Gravitas/Queries/3D/Sweeps/SweptSphereQueryWorker.cs`
...
- Test: `../Gravitas/tests/Gravitas.Tests/Queries/GravitasQuery3DServiceSweepTests.cs`
- Test: `../Gravitas/tests/Gravitas.Tests/MixedDimensions/MixedQueryCcdTests.cs`

- [ ] Add failing extreme crossings, first-root-rejected/second-root-valid,
      tangency/miss, radius-overflow, one-raw-direction, authored endpoint,
      and intersections-disabled parity regressions.
- [ ] Replace duplicated circle/sphere quadratics with the FixedMathSharp
      interval and cross-section APIs, passing radius expansion separately.
- [ ] Preserve spatial-distance units, deterministic feature order, exact
      start/end containment, and authored endpoint reconstruction.
- [ ] Do not route arbitrary segment/mesh-edge projections through already
      rounded perpendicular vectors; keep them queued for the dedicated
      finite-axis geometry task.
- [ ] Run full Release, focused coverage, allocation regressions, and relevant
      query benchmarks.

## Task 5: Closure

- [ ] Update `MIGRATION.md`, complexity exceptions, both issue trackers, and
      the Gravitas active queue with exact completed/deferred scope.
- [ ] Re-achieve 100% FixedMathSharp line/branch/method coverage without
      coverage-only or API-shape tests.
- [ ] Obtain an independent correctness/performance review.
