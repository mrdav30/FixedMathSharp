...
/// transformations used by GJK/EPA-based wide (batched) collision detection routines.
/// </summary>
internal static partial class WideGeometry
{
    internal static readonly Signed192 CenteredAxisDenominator =
        Signed192.Signed(FixedMath.ONE_L * 2L);

    #region Nested Types

    private enum CenteredAxisSupportKind
    {
        Capsule,
        Cylinder,
        Cone,
        ...
        Fixed64 thirdZ)
    {
        ...
        Signed320 tripleProduct = WideArithmetic.AddSigned320(
            WideArithmetic.AddSigned320(
                WideArithmetic.MultiplySigned192(Signed192.Signed(firstX.m_rawValue), minorX),
                WideArithmetic.MultiplySigned192(Signed192.Signed(firstY.m_rawValue), minorY)),
            WideArithmetic.MultiplySigned192(Signed192.Signed(firstZ.m_rawValue), minorZ));
    ...
    internal static bool TryGetMagnitudeCeiling(
        Fixed64 x,
        Fixed64 y,
        Fixed64 z,
        out Fixed64 magnitude)
    {
        Signed192 rawX = Signed192.Signed(x.m_rawValue);
        Signed192 rawY = Signed192.Signed(y.m_rawValue);
        Signed192 rawZ = Signed192.Signed(z.m_rawValue);
        Signed320 squaredMagnitude = WideArithmetic.AddSigned320(
            WideArithmetic.AddSigned320(
                WideArithmetic.MultiplySigned192(rawX, rawX),
                WideArithmetic.MultiplySigned192(rawY, rawY)),
            WideArithmetic.MultiplySigned192(rawZ, rawZ));
        Signed192 root = WideArithmetic.GetFloorSquareRoot(
            squaredMagnitude,
            out Signed192 remainder);
        if (!remainder.IsZero)
        {
            root = WideArithmetic.AddSigned192(
                root,
                Signed192.Signed(1L));
        }

        if ((root.High | root.Middle | (root.Low >> 63)) != 0UL)
        {
            magnitude = default;
            return false;
        }

        magnitude = Fixed64.FromRaw((long)root.Low);
        return true;
    }

    internal static FixedQuaternion GetCanonicalAxisRotation(
        Vector3d normalizedAxis)
    {
        if (normalizedAxis == Vector3d.Up)
            return FixedQuaternion.Identity;
        if (normalizedAxis == Vector3d.Down)
        {
        ...
        return new FixedQuaternion(
            ...
            Fixed64.Zero,
            -normalizedAxis.X,
            Fixed64.One + normalizedAxis.Y).Normalized;
    }

    /// <summary>
    /// Returns one outward-rounded endpoint of a centered finite-axis extent,
    /// clipped to the representable scalar domain.
    /// </summary>
    internal static Fixed64 GetCenteredFiniteAxisBoundClippedToDomain(
        Fixed64 center,
        Fixed64 axisComponent,
        Fixed64 fullAxisLength,
        Fixed64 radialExtent,
        bool minimum)
    {
        Signed320 centerNumerator = WideArithmetic.MultiplySigned192(
            Signed192.Signed(center.m_rawValue),
            CenteredAxisDenominator);
        Signed320 extentNumerator = WideArithmetic.AddSigned320(
            WideArithmetic.MultiplySigned192(
                Signed192.Signed(axisComponent.Abs().m_rawValue),
                Signed192.Signed(fullAxisLength.m_rawValue)),
            WideArithmetic.MultiplySigned192(
                Signed192.Signed(radialExtent.m_rawValue),
                CenteredAxisDenominator));
        Signed320 numerator = minimum
        ...
        return GetRationalBoundClippedToDomain(
            numerator,
            CenteredAxisDenominator,
            minimum);
    }

    internal static Fixed64 GetRationalBoundClippedToDomain(
        Signed320 numerator,
        Signed192 denominator,
        bool minimum)
    {
        ...
        // Clip the exact endpoint before round-to-even so the subsequent
        // one-unit outward correction cannot cross the scalar domain.
        ...
        Fixed64 bound = Fixed64.GetSignedRawRatio(
            numerator,
            denominator);
        Signed320 represented = WideArithmetic.MultiplySigned192(
            Signed192.Signed(bound.m_rawValue),
            denominator);
        int comparison = WideArithmetic.SubtractSigned320(represented, numerator).Sign;
