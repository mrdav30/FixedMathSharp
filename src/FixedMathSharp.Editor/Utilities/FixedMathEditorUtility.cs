...
namespace FixedMathSharp.Unity.Editor
{
    public static class FixedMathEditorUtility
    {
        ...
        {
            return (Fixed64)EditorGUI.DoubleField(position, GUIContent.none, Math.Round((double)value, 4, MidpointRounding.AwayFromZero));
        }

        public static Fixed64 FixedNumberField(Rect position, GUIContent label, long value)
        {
            return (Fixed64)EditorGUI.DoubleField(position, label, Math.Round((double)value, 4, MidpointRounding.AwayFromZero));
        }
